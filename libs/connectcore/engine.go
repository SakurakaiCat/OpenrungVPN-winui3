...
//
// This package is the root of the nested connectcore module (ADR-001 D3), so
// mobile consumers can fetch it: it may depend only on its own subpackages
// and the sibling nested modules (brokerapi, wsscore, punchcore) — never on
// the root module's internal packages. Relay wire types come from brokerapi's
// exported relay schema, and the QUIC punch transport stays behind the
// PunchEstablisher seam (implemented by the root module's internal/punch).
package connectcore
...
type connection struct {
	...
	candidates    []brokerapi.RelayDescriptor
...
type ConnectionInfo struct {
	Relay     brokerapi.RelayDescriptor
	Transport string // brokerapi.TransportDirect, "punch", or "wss"
...
func (s *Engine) candidatesFor(resp brokerapi.RelayListResponse, target RelayTarget) ([]brokerapi.RelayDescriptor, string, error) {
...
func (s *Engine) runLadder(ctx context.Context, conn *connection, cands []brokerapi.RelayDescriptor, port int) (*candidateResult, error) {
...
func (s *Engine) attemptCandidate(ctx context.Context, conn *connection, cand brokerapi.RelayDescriptor, port, attempt int) (*candidateResult, error) {
